...
\begin{proposition}
  	If the sequence $\bmu^1,\bmu^2,\dots$ generated by Algorithm \ref{alg:do} converges weakly to an equilibrium $\bmu^\star$, then $\lim_j \bigl(U_i(x_i^{j+1},\bmu_{-i}^j)-U_i(\bmu^j)\bigr)=0,$ for each $i\in N.$
\end{proposition}
\begin{proof}
...
The choice of computational methods should reflect the properties of a~continuous game, since the efficiency of methods for solving the master problem and subproblem is the decisive factor for the overall performance and precision of Algorithm \ref{alg:do}. For example, polymatrix games are solvable in polynomial time \cite{Cai16}, whereas finding even an approximate Nash equilibrium of a finite general-sum game is a very hard problem \cite{Daskalakis09}. As for the solution of the oracles, the best response computation can be based on global solvers for special classes of utility functions.
