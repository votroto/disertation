...
\begin{enumerate}
    \item Every compact strategy space $X_i\subseteq \R^{m_i}$ is a \emph{basic semi-algebraic set}; that is,
    \begin{equation}\label{semi}
        X_i = \{\bx\in \R^{m_i} \mid g_{ik}(\bx)\geq 0, \; k\in K_i \},
    \end{equation}
    where each $g_{ik}$ is a real polynomial in $m_i$ variables and $K_i\neq\emptyset$ is a finite index set. We also assume the standard archimedean certificate on compactness on the sets; e.g., that one of the constraints has the form $g_{ik}(\bx)=a-\norm{\bx}^2$ for some $k\in K_i$ and some positive $a$. 
...
\end{enumerate}
These three assumptions define a \emph{polynomial zero-sum network game} $\calG$. We remark that the first assumption is very mild --- many sets important for applications are in fact the solution sets of finitely many polynomial inequalities. In particular, a compact semi-algebraic set is not necessarily convex or connected. Moreover, Glicksberg's theorem (\ref{thm:Glick}) can be significantly strengthened for polynomial games --- each polynomial game has NE in which every player mixes only among finitely many pure strategies \cite{DresherKarlinShapley50,SteinOzdaglarParrilo08}.
...
We then iteratively solve each level of the hierarchy until all sequences $\hat{\by}_i$ have a representing atomic measure, which we extract using the algorithm of \textcite[Algorithm 6.9]{Lasserre2015}. With the strategies of other players fixed, the best response condition is equivalent to certifying optimality for a polynomial over a semialgebraic set.