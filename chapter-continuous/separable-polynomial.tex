...
\begin{enumerate}
    \item Every compact strategy space $X_i\subseteq \R^{m_i}$ is a \emph{basic semi-algebraic set} $\calS(\bg_i)$ where each $g_{ik}$ is a real polynomial in $m_i$ variables and $K_i\neq\emptyset$ is a finite index set. We also assume the standard archimedean certificate on compactness \ref{def:archimedean} on the sets.
