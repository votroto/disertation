...
\]  
We say that $\calG$ is \emph{finite} if each strategy set $X_i$ is finite.
...
\begin{definition}\label{def:ne}
    A profile of mixed strategies $\bmu^*\in\bM$ is a Nash equilibrium if, and only if
    \begin{equation*}
        U_i(\bmu^*)=\max_{x_i\in X_i} U_i(x_i,\bmu_{-i}^*), \qquad \forall i\in N.
    \end{equation*}
\end{definition}

\begin{theorem}[Glicksberg]\label{thm:Glick}
