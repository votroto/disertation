...
\begin{equation} \label{def:game}
    \calG=\langle N,(X_i)_{i\in N},(u_i)_{i\in N} \rangle
\end{equation}
is called a \emph{separable network game with compact strategy sets}. Throughout the paper we refer to game (\ref{def:game}) as a ``network game'' for short. We say that $\calG$ is 
\begin{itemize}
...
\end{itemize}
The class of network games most studied in the literature is the class of zero-sum polymatrix games. The ``global'' zero-sum assumption makes such finite games computationally tractable \cite{Cai16,FPNet,Bailey19,Rubinstein,CaiDaskalkis}.

\begin{example}
Let $N=\{1,2,3,4\}$ and the graph $(N,E)$ be the cycle on Figure \ref{fig:4}. This is the smallest example of a network game such that each player interacts only with a strict subset of other players. For example, the utility function of player $3$ is $u_3(x_1,x_2,x_3,x_4)=u_{31}(x_3,x_1)+u_{34}(x_3,x_4)$. The zero-sum condition for this game can be formulated as $u_3=-u_1-u_2-u_4$.
\end{example}
...
\begin{theorem}\label{thm:opti}
    For every profile of mixed strategies $\bmu^* \in \bcalP$ in a zero-sum continuous network game $\calG$, the following are equivalent:
    \begin{enumerate}
        \item $\bmu^*$ is a Nash equilibrium of game $\calG$.
        \item $(\bmu^*, \bw^*)$ is an optimal solution to (\ref{P}), where
        \begin{equation}\label{def:wi}
            w_i^* = \max_{x_i \in S_i} U_i(x_i, \bmu_{-i}^*), \qquad i \in N.
        \end{equation}
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    By Theorem \ref{thm:Glick}, there exists an NE $\bmu^*$ such that $U_i(\bmu^*) = \max_{x_i \in X_i} U_i(x_i, \bmu_{-i}^*) \; \forall i \in N$. Let $\bmu$ be an NE and $\bw^*_i$ be as in condition (\ref{def:wi}), then the inequalities in (\ref{ineq1}) become equalities, and the lower bound of zero is achieved in (\ref{ineq3}).

    We have shown that NE correspond to optimal solutions. Conversely, consider any optimal solution $(\bmu^*, \bw^*)$. Notice that $w_i^*$ must be tight upper bounds on $U_i(\cdot, \bmu_{-i}^*)$. By the zero-sum property, $\sum_{i \in N} U_i(\bmu) = 0$ for any $\bmu$. If $\bmu^*$ were not an NE, then there exists a player $i$ such that $\max_{x_i \in X_i} U_i(x_i, \bmu_{-i}) > U_i(\bmu)$. This implies that 
    \[
