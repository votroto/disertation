...
\todo[inline]{rerun experiments including warmstart}
\todo[inline]{reformat experiments to make them shorter (unnecessary repetition)}
\todo[inline]{include the results for discontinuous games}
...
\iffalse
in some situations it is a great benefit to be able to directly provide an oracle
specific to a given game, as in some games best responses can be computed in closed form.
Still in other cases, players may need to solve common software-engineering problems such
as computing the shortest path through a graph, in which case the oracles
may be implemented in terms of existent highly optimized algorithms.

It turns out however that for some games it is sufficient to
lean on the tools of mathematical optimization with sufficiently generic global solvers.
As our experiments show, a many continuous games from the literature can be solved in this way.
\fi
