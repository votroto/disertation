...
This section presents our experimental evaluation of our generic Double Oracle algorithm implementation over the dataset of games in Appendix \ref{appendix:games}. 

\iffalse
The experimental results reveal significant computational improvements over state-of-the-art methods. To illustrate the performance gains, we highlight Example \ref{ex.zhou19.5.1}, where our algorithm identifies the pure strategy Nash equilibrium in 0.086 seconds, representing a speedup of over three orders of magnitude compared to the 184.5 seconds reported in the original work. This substantial improvement is achieved despite our implementation being a general-purpose proof-of-concept, suggesting that further optimizations could yield even greater performance gains.

The consistent performance improvements observed across our benchmark collection indicate that the Double Oracle framework provides a robust and efficient foundation for solving a broad class of game-theoretic problems, offering significant advantages over specialized algorithms in terms of both computational efficiency and generality.
\fi

\subsection{Experiments on General Blotto}
The utility functions in \emph{General Blotto} games are sums of non-linearly weighted margins of victory over sets of fronts. The family of weighing functions described by \textcite{Golman2009} is
\[
...
While products, exponents, sums and differences should be available through most modeling interfaces for non-linear problems; neither $\operatorname{sgn}$, nor the absolute value is available via the non-linear input for \emph{Gurobi}. We attempted to model the absolute value as $\sqrt{z^2}$ and the sign function as $\frac{z}{\sqrt{z^2}}$.
...
\begin{figure}
    ...
    \addplot[index of colormap={3 of Set1}, ] {sign(x)*abs(x)^2};
    \addlegendentry{$p=2.0$}
  
    \addplot[index of colormap={2 of Set1}, ] {sign(x)*abs(x)^1};
    \addlegendentry{$p=1.0$}
    
    \addplot[index of colormap={1 of Set1}, ]    {sign(x)*abs(x)^0.5};
    ...
    \addplot[index of colormap={4 of Set1}, only marks,mark=o] coordinates {(0,1)}; % open circle
    \addplot[index of colormap={4 of Set1}, only marks,mark=o] coordinates {(0,-1)}; % open circle
    \addplot[index of colormap={4 of Set1}, only marks,mark=*] coordinates {(0,0)}; % filled circle
...
For each Example \ref{ex.golman09.isolated}--\ref{ex.golman09.sets}, we computed equilibria in a game with 5 fronts for each of the values $p\in\{0.5, 1, 2\}$ as representative cases of each function behavior. The functions are shown in \cref{fig:blotto.weighing.functions}, and our representation is:
\[
f(z) = \begin{cases}
\frac{z}{\sqrt[4]{z^2}} & \quad p = 0.5 \\
z & \quad p = 1 \\
z\,\sqrt{z^2} & \quad p = 2.
\end{cases}
\]

In the smooth case of $p=2$, \emph{Double Oracle} was able to recover a pure equilibrium for the game with  isolated fronts, and mixed equilibria in the other two games where pure equilibria do not exist.
\begin{description}
...
\item[Result of Example \ref{ex.golman09.pairs}] For all pairs, DO computed $\epsilon$-NE:
...
in two iterations and a time of  $0.018\,s$.
\item[Result of Example \ref{ex.golman09.sets}] For all sets, DO computed $\epsilon$:
$x_1^\star \approx \left(\frac{1}{5}, \frac{1}{5}, \frac{1}{5}, \frac{1}{5}, \frac{1}{5}\right)$, $x_2^\star \approx \left(\frac{1}{5}, \frac{1}{5}, \frac{1}{5}, \frac{1}{5}, \frac{1}{5}\right)$
in two iterations and $1.5\,s$.
\end{description}

The utility functions are continuous but not differentiable for $0<p<1$; However, our reformulation contains a discontinuity at $f(0)$, which we believe to be the reason why \emph{Gurobi} was unable to converge for all three \emph{Blotto} games for this parameter value. We will return to this case along with the discontinuous game of \emph{Colonel Blotto} with $p=0$ later in section \ref{sec:bad}.


To illustrate the complexity of the utility functions in \cref{ex.golman09.sets} where all sets of fronts are equally important, we write it out explicitly (we break our rule of consistent notation due to a lack of space, and denote the variables of player 1 as $x_1,\dots,x_5$, and player 2 as $y_1,\dots,y_5$.):
{\tiny
\begin{align*}
u_1(x,y) 
    &= \operatorname{sgn}(x_1x_3x_4 - y_1y_3y_4)\abs{x_1x_3x_4 - y_1y_3y_4}^p 
    + \operatorname{sgn}(x_1x_2x_5 - y_1y_2y_5)\abs{x_1x_2x_5 - y_1y_2y_5}^p 
    + \operatorname{sgn}(x_3x_4x_5 - y_3y_4y_5)\abs{x_3x_4x_5 - y_3y_4y_5}^p\\ 
    &+ \operatorname{sgn}(x_1x_2x_3x_5 - y_1y_2y_3y_5)\abs{x_1x_2x_3x_5 - y_1y_2y_3y_5}^p 
    + \operatorname{sgn}(x_1x_3x_4x_5 - y_1y_3y_4y_5)\abs{x_1x_3x_4x_5 - y_1y_3y_4y_5}^p \\
    &+ \operatorname{sgn}(x_1x_2x_4x_5 - y_1y_2y_4y_5)\abs{x_1x_2x_4x_5 - y_1y_2y_4y_5}^p 
    + \operatorname{sgn}(x_2x_3x_4 - y_2y_3y_4)\abs{x_2x_3x_4 - y_2y_3y_4}^p 
    + \operatorname{sgn}(x_5 - y_5)\abs{x_5 - y_5}^p \\
    &+ \operatorname{sgn}(x_4 - y_4)\abs{x_4 - y_4}^p 
    + \operatorname{sgn}(x_2x_4 - y_2y_4)\abs{x_2x_4 - y_2y_4}^p 
    + \operatorname{sgn}(x_1 - y_1)\abs{x_1 - y_1}^p 
    + \operatorname{sgn}(x_2x_3 - y_2y_3)\abs{x_2x_3 - y_2y_3}^p \\
    &+ \operatorname{sgn}(x_2x_4x_5 - y_2y_4y_5)\abs{x_2x_4x_5 - y_2y_4y_5}^p 
    + \operatorname{sgn}(x_3x_5 - y_3y_5)\abs{x_3x_5 - y_3y_5}^p 
    + \operatorname{sgn}(x_1x_2x_4 - y_1y_2y_4)\abs{x_1x_2x_4 - y_1y_2y_4}^p \\
    &+ \operatorname{sgn}(x_1x_5 - y_1y_5)\abs{x_1x_5 - y_1y_5}^p 
    + \operatorname{sgn}(x_3 - y_3)\abs{x_3 - y_3}^p 
    + \operatorname{sgn}(x_1x_2x_3 - y_1y_2y_3)\abs{x_1x_2x_3 - y_1y_2y_3}^p \\
    &+ \operatorname{sgn}(x_3x_4 - y_3y_4)\abs{x_3x_4 - y_3y_4}^p 
    + \operatorname{sgn}(x_1x_3 - y_1y_3)\abs{x_1x_3 - y_1y_3}^p 
    + \operatorname{sgn}(x_1x_2x_3x_4x_5 - y_1y_2y_3y_4y_5)\abs{x_1x_2x_3x_4x_5 - y_1y_2y_3y_4y_5}^p \\
    &+ \operatorname{sgn}(x_4x_5 - y_4y_5)\abs{x_4x_5 - y_4y_5}^p 
    + \operatorname{sgn}(x_2x_3x_5 - y_2y_3y_5)\abs{x_2x_3x_5 - y_2y_3y_5}^p 
    + \operatorname{sgn}(x_2x_3x_4x_5 - y_2y_3y_4y_5)\abs{x_2x_3x_4x_5 - y_2y_3y_4y_5}^p \\
    &+ \operatorname{sgn}(x_1x_4 - y_1y_4)\abs{x_1x_4 - y_1y_4}^p 
    + \operatorname{sgn}(x_2x_5 - y_2y_5)\abs{x_2x_5 - y_2y_5}^p 
    + \operatorname{sgn}(x_1x_4x_5 - y_1y_4y_5)\abs{x_1x_4x_5 - y_1y_4y_5}^p \\
    &+ \operatorname{sgn}(x_1x_3x_5 - y_1y_3y_5)\abs{x_1x_3x_5 - y_1y_3y_5}^p 
    + \operatorname{sgn}(x_2 - y_2)\abs{x_2 - y_2}^p 
    + \operatorname{sgn}(x_1x_2x_3x_4 - y_1y_2y_3y_4)\abs{x_1x_2x_3x_4 - y_1y_2y_3y_4}^p \\
    &+ \operatorname{sgn}(x_1x_2 - y_1y_2)\abs{x_1x_2 - y_1y_2}^p.
\end{align*}
}
...
\end{align*}
in $4$ iterations, and a time of $0.016\,s$. Despite the simplicity of this \emph{guessing game}, it is interesting for the \emph{DO} algorithm. The \emph{avoiding} player quickly finds both $1$ and $-1$ as best responses, after which the pure strategy of the \emph{chasing} player oscillates to find the center of mass of the \emph{avoiding} player. The \emph{chasing} player will never reach the point $0$, and so will never play a pure strategy, except when initialized at that point.
\todo[inline]{check and add supporting statements with lots of maths :) }
...
The following examples are two-player zero-sum rational games, that originally served as experiments for the algorithm developed by \textcite{zhou.wang.ea:saddle}. Their algorithm can prove the existence of (global) saddle points, i.e. pure Nash Equilibria.
...
\begin{description}
\item[Result of Example \ref{ex.zhou19.5.1}]
After $0.086\,s$ and $4$ iterations, DO converged to the $\epsilon$-NE:
\begin{align*}
x_{1}^\star \approx  (0.32, 0.32, 0.37) ,\;
x_{2}^\star \approx  (0, 1, 0) .
\end{align*}
...
\end{align*}
\item[Result of Example \ref{ex.zhou19.5.5.i}]
After $0.17\,s$ and $6$ iterations, DO converged to the $\epsilon$-NE:
\begin{align*}
\mu_1^\star &\approx 0.283\delta_{ (-0.71, -0.71) } + 0.29\delta_{ (-0.71, -0.71) } + 0.427\delta_{ (0.71, 0.71) },\\ 
\mu_2^\star &\approx \delta_{ (0, 0) }.
\end{align*}
\item[Result of Example \ref{ex.zhou19.5.5.ii}]
After $0.34\,s$ and iterations: $10$ DO converged to the $\epsilon$-NE:
\begin{align*}
\mu_1^\star &\approx 0.456\delta_{ (0.49, 0.43, 0.35) } + 0.544\delta_{ (0.46, 0.47, 0.34) },\\ 
\mu_2^\star &\approx 0.365\delta_{ (0.64, 0.59, 0.49) } + 0.635\delta_{ (0.67, 0.54, 0.5) }.
\end{align*}
\item[Result of Example \ref{ex.zhou19.5.6.ii}]
After $0.072\,s$ and $3$ iterations, DO converged to the $\epsilon$-NE:
\begin{align*}
\mu_1^\star &\approx \delta_{ (0.41, 0.41, 0.82) },\\ 
\mu_2^\star &\approx 0.006\delta_{ (-0.38, -0.84, -0.38) } + 0.994\delta_{ (-0.39, -0.84, -0.39) }.
\end{align*}
\item[Result of Example \ref{ex.zhou19.5.7.i}]
After $0.047\,s$ and $2$ iterations, DO converged to the $\epsilon$-NE:
\begin{align*}
x_{1}^\star \approx  (0.41, 0.64, 0.64) ,\;
x_{2}^\star \approx  (0.54, 0.84) .
\end{align*}
\item[Result of Example \ref{ex.zhou19.5.7.ii}]
After $0.052\,s$ and $3$ iterations, DO converged to the $\epsilon$-NE:
\begin{align*}
x_{1}^\star \approx  (0.95, 0.22, 0.22) ,\;
x_{2}^\star \approx  (1, 0, 0) .
\end{align*}
\end{description}

\subsection{Games from Classical Literature}
The following games appear in books from the $1950$s and $60$s, where they were used as demonstrations that the solutions of continuous games may be supported on anything from singleton sets to Cantor sets and whole intervals. \todo[inline]{Dresher62 pg 111 failed!!!}
...
\end{align*}
\item[Result of Example \ref{ex.dresher61.pg111}] did not finish. Root relaxation failed.
...
The following games are a mix from game theory applied to communication networks and security games, as well as others that did not fit elsewhere.
