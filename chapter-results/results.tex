...
For each Example \ref{ex.golman09.isolated}--\ref{ex.golman09.sets}, we computed equilibria in a game with 5 fronts for each of the values $p\in\{0.5, 1, 2\}$ as representative cases of each behavior as shown in \cref{fig:blotto.weighing.functions}.
...
The utility functions are continuous but not differentiable for $0<p<1$; Since \emph{Gurobi} does not accept neither the sign function nor absolute value through its non-linear interface. Attempting to model the utility as $\frac{z}{\sqrt[4]{z^2}}$ introduces a discontinuity which \emph{Gurobi} is able to avoid; however, the oracle did not find a best response within two minutes. The local solver \emph{Ipopt} was failed to compute a solution as well, since the utility is not differentiable. The global solver \emph{Couenne} via \emph{AMPL} depends on \emph{Ipopt} and was similarly unsuccessful.
