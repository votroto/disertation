The moment-SOS hierarchies are a basis of methods for constrained polynomial optimization. More specifically, the methods will be able to find the certifiably global minimum of a polynomial over a compact set defined by polynomial inequalities. In some instances, they can even recover some minimizers. All of this is achievable under some mild assumptions on the structure of the feasible set, such as having a bound on the norm of the feasible elements. Under such constraints, each level of the hierarchy corresponds to a strongly dual pair of semidefinite programs which provide a monotonically nondecreasing sequence of lower bounds that generically converge \cite{Nie2014} to the optimum. The current downsides of the hierarchies are that: the bounds on the convergence is not yet well understood; semidefinite solvers are not as well optimized for this task; and the recovery of minimizers of sufficient precission may be practically challenging.
